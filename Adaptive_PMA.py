...
class Adaptive_PMA:
    ...
    def recursiveUnevenPlace(self, theElems, rangeLeft, rangeRight):
        ...
        optValue = abs(leftSum/(leftCap - splitNum + 1) - rightSum/(rightCap-(m-splitNum) + 1))
    ...
    def noisySlot(self, element, slot):
        #Randomizarion Addition
        if self.capacity - slot > 1:

            noiseSize = round(np.log2(self.capacity))
            noiseArray = [i for i in range(-noiseSize, noiseSize + 1)]
            
            for i in range( max(0, slot - noiseSize), min(self.capacity, slot + noiseSize + 1)):
                if self.theArray[i] is not None:

                    if self.theArray[i] > element:

                        for k in range(i, slot + noiseSize + 1):
                            toRemove = k - slot
                            if toRemove in noiseArray:
                                noiseArray.remove(toRemove)
            
                        break

                    elif self.theArray[i] < element:

                        for k in range( slot - noiseSize, i+ 1):
                            toRemove = k - slot
                            if toRemove in noiseArray:
                                noiseArray.remove(toRemove)
            ...
            if noiseArray:        
                noise = self.rng.choice(noiseArray)
                if slot + noise < self.capacity:
                    slot += noise

        return slot
    
    def insert(self, element):
        baseSlot = self.getSlot(element)

        leaf_left = self.leftRangeBoundary(baseSlot, 0)
        leaf_right = leaf_left + self.smallestRangeSize
        noisySlot = self.noisySlot(element, baseSlot)
        noisySlot = max(leaf_left, min(leaf_right - 1, noisySlot))

        if self.rangeIsInThreshold(baseSlot, 0):
            final_slot = noisySlot
        else: 
            self.makeRoom(baseSlot) 
            final_slot = self.getSlot(element)

        for i in reversed(range(0, final_slot + 1)):
            if self.theArray[i] is not None:
                self.predictor.insert(self.theArray[i])
                break
        self.insertIntoLeaf(element, final_slot)
    
        if self.lastElement is None or element > self.lastElement:
            self.lastElement = element
        if self.firstElement is None or element < self.firstElement:
